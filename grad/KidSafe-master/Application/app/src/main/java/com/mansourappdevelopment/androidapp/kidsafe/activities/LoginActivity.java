...
public class LoginActivity extends AppCompatActivity implements OnPasswordResetListener {
    ...
    private void checkMode(String ignored) {

        FirebaseUser user = auth.getCurrentUser();
        if (user == null) {
            Toast.makeText(this, "Session lost, login again", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        DatabaseReference path1 =
                firebaseDatabase.getReference("parents").child(uid);

        DatabaseReference path2 =
                firebaseDatabase.getReference("users").child("parents").child(uid);
